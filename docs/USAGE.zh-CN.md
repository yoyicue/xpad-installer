...
```shell
cd xpad-installer-v0.2.5-android-arm64
...
从 v0.2.5 起，正式 anchor 使用 versionCode 2。这样遇到“attribution 已持久化、alias
尚未物化”的现场时，修复会用 full update 形成一次真实升级；同版本修复仍使用
inherit-existing。提交后最多用五秒短轮询同时验证 alias、实际 UID 和 attribution；
健康后立即结束，不固定等待。
